...
namespace WebRTC.Servers.ServerFxns.Core
{
    public class TURNClient
    {
        ...
        public async Task<Classes.IceServer[]> GetXirSysIceServersAsync(string urlPath, string authorization)
        {
                ...
                if (iceServersJson != null)
                {
                    ...
                    if (serversObject != null)
                    {
                        ...
                        var groupedServers = xirSysServers.GroupBy(
                            ...
                            (username, cred_url) =>
                            new Classes.IceServer(urls: cred_url.Select(cu => cu.Url).ToArray(), username: username, password: cred_url.Select(cu => cu.Credential).Distinct().FirstOrDefault())
                        ).ToArray();
