...
namespace WebRTC.Servers.ServerFxns.Core
{
    public class RTCClient : IRTCClient<RoomConnectionParameters>, IWebSocketChannelEvents
    {
        ...
        public void SendSdpOffer(SessionDescription sdp)
        {
            _executor.Execute(() =>
            {
        ...
        public void SendSdpAnswer(SessionDescription sdp)
        {
            _executor.Execute(() =>
            {
                if (_connectionParameters.IsLoopBack)
        ...
        public void SendLocalIceCandidateRemovals(IceCandidate[] candidates)
        {
            _executor.Execute(() =>
            {
                ...
                if (_initiator)
                {
                    ...
                    if (_connectionParameters.IsLoopBack)
                    {
                        _signalingEvents.OnRemoteIceCandidateRemoved(candidates);
                    }
                }
                else
                {
        ...
        public void OnWebSocketMessage(string message)
        {
            ...
            switch (msg.SignalingMessageType)
            {
                ...
                case SignalingMessageType.CandidateRemoval:
                    ...
                    _signalingEvents.OnRemoteIceCandidateRemoved(candidates.IceCandidates);
                    break;
        ...
        private void SendPostMessage(MessageType messageType, string url, string message)
        {
            ...
            var httpConnection = new AsyncHttpURLConnection(HttpMethodType.Post, url, message, ((response, errorMessage) =>
            {
                _executor.Execute(() =>
                {
